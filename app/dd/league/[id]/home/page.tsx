...
import FantasyHomeClient from './FantasyHomeClient';

export const metadata = {
  title: 'Fantasy Home — DiamondDraft',
};
...
export default async function FantasyHomePage({
  ...
  const league = await queryOne<{ id: string; name: string; is_public: boolean }>(
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <FantasyHomeClient leagueId={league.id} leagueName={league.name} />
      </main>
      <Footer />
    </>
  );
