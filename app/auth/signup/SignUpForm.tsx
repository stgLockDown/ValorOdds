...
export default function SignUpForm({ next }: { next: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const s = await signIn('credentials', { email, password, redirect: false, callbackUrl: next });
      if (s?.error) {
  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="space-y-4">
        <div>
          <label htmlFor="displayName" className="label">Display name <span className="text-brand-muted">(optional)</span></label>
          <input
            id="displayName"
            type="text"
            className="input"
